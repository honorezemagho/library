...
const helpers = {
    cutText(text, length) {
        if (text.split(' ').length > 1) {
            ...
            let splitTimport moment from 'moment'
...
const helpers = {
    ...
    diffTimeByNow(time) {
        let startDate = moment(moment().format('YYYY-MM-DD HH:mm:ss').toString())
        let endDate = moment(moment(time).format('YYYY-MM-DD HH:mm:ss').toString())
