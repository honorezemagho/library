<div>
                    ...
                    <tr>
                        ...
                        <td class="border-b w-5">
                            <div  x-data="{ isTouch: false }" class="flex sm:justify-center items-center">
                                <a href="{{ route('book-items.index',['id' => $book->id]) }}" class="flex items-center text-theme-4 mr-3">
    ...
    <x-jet-dialog-modal :maxWidth="'6xl'" class="z-40" wire:model="showModalForm">
        ...
        <x-slot name="title">Updating a Book</x-slot>   
        @else
            <x-slot name="ico">
            ...
            <x-slot name="title">Creation of a  new Book</x-slot>  
        @endif
        <x-slot name="close">
     ...
     <x-slot name="footer">
       ...
       @else
       <x-jet-button wire:click="storeBook"> {{ __('Create') }}</x-jet-button>
       @endif
     </x-slot>
    </x-jet-dialog-modal>
    
    <x-jet-confirmation-modal wire:model="showDeleteModalForm">
        <x-slot name="title">Deletion of a Book</x-slot>
        <x-slot name="close">
            <a x-on:click.prevent @click="@this.closeModal()" href=""> 
                <svg xmlns="http://www.w3.org/2000/svg" class="mt-1 ml-2 text-white h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
        ...
        <x-slot name="content">
            <div class="space-y-8 text-2xl divide-y divide-gray-200">
                <div class="p-5 text-center"> <i data-feather="x-circle" class="w-16 h-16 text-theme-6 mx-auto mt-3"></i>
                    @if($this->book_id)
                    <p>Do you really want to delete this <span class="text-gray-900 text-bold">Book ?</span> This process cannot be undone.</p>
                    @endif
                </div>      
            </div>
        </x-slot>
        <x-slot name="footer">
           <x-jet-button wire:click="deleteBook({{ $this->book_id }})" class="bg-red-700">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 011<div>
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Book Management</h2>
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            <x-jet-button wire:click="showCreateBookModal" class=" bg-theme-1 shadow-md mr-2 text-center">Add New Book</x-jet-button>    
