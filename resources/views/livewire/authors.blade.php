<div>
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Authors Management</h2>
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            <x-jet-button wire:click="showCreateAuthorModal" class=" mr-2 text-center bg-blue-800">Add New Author</x-jet-button>
        ...
        <table class="table table-report table-report--bordered display datatable w-full">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="border-b">
                        <div class="font-medium whitespace-no-wrap"> {{ $author->name }}</div>
                    </td>
                  
                    <td class="border-b w-5">
                        <div  x-data="{ isTouch: false }" class="flex sm:justify-center items-center">
                            <a x-on:click.prevent wire:click="showEditAuthorModal({{ $author->id }})" @touchstart.prevent="isTouch = true" class="flex items-center text-theme-4 mr-3" href="">
                                <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg> Edit
                            </a>
                            <a x-on:click.prevent wire:click="showDeleteAuthorModal({{ $author->id }})" class="flex items-center text-theme-6" href="">
                                <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<x-jet-dialog-modal class="z-40" wire:model="showModalForm">
    ...
    <x-slot name="close">
     ...
     <x-slot name="footer">
       ...
       @else
       <x-jet-button wire:click="storeAuthor">Store</x-jet-button>
       @endif
     </x-slot>
 </x-jet-dialog-modal>
    ...
    <x-slot name="content">
        <div class="space-y-8 text-2xl divide-y divide-gray-200">
            <div class="p-5 text-center"> <i data-feather="x-circle" class="w-16 h-16 text-theme-6 mx-auto mt-3"></i>
                @if($this->author_id)
                <p>Do you really want to delete this <span class="text-gray-900 text-bold">Author ?</span> This process cannot be undone.</p>
            @endif
            </div>      
        </div>
    </x-slot>
       ...
       <x-slot name="footer">
           <x-jet-button wire:click="deleteAuthor({{ $this->author_id }})" class="bg-red-700">
