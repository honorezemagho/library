                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-200 text-sm leading-5 font-medium">
                                    ...
                                    @if(!is_null($bug->resolved_by) && !$bug->status)
                                        <form method="post" action="{{route('approve-solution')}}">
                                            @csrf
                                        <input type="hidden" name="id" value="{{$bug->id}}">

                                        <button type="submit" class=" {{!$bug->status  ? 'bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded' : '' }}">
                                        {{!$bug->status ? "Approve" : ''}}
                                            </button>
                                    ...
                                    @else
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-green-800">Waiting for Resolve</span>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
