      ...
      <div class="mt-5 md:mt-0 md:col-span-2 flex align-items-center px-3">
        <form action="{{ route('admin.books.update', $book->id) }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="shadow sm:rounded-md sm:overflow-hidden">
            ...
            <input type="text" name="title" id="title" required  value="{{ $book->title }}" autocomplete="title" class="form-input mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow sm:text-sm border-gray-400 rounded-md">
                    @error('title')<span class="mt-2 text-sm text-red-500">{{ $message }}</p>@enderror
                </div>
                <div class="col-span-6 sm:col-span-3">
                    <label for="author" class="block text-sm font-medium text-gray-700">Author</label>
                    <input type="text" name="author" id="author" value="{{ $book->author }}" required autocomplete="author" class="form-input mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow sm:text-sm border-gray-400 rounded-md">
                    @error('author')<span class="mt-2 text-sm text-red-500">{{ $message }}</p>@enderror
                </div>
                <div class="col-span-6 sm:col-span-3">
                    <label for="isbn" class="block text-sm font-medium text-gray-700">ISBN</label>
                    <input type="text" name="isbn" id="isbn" value="{{ $book->isbn }}" required autocomplete="isbn" class="form-input mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow sm:text-sm border-gray-400 rounded-md">
                ...
                <div class="col-span-6 sm:col-span-3">
                    ...
                    <select name="category_id" id="category_id" required class="form-input mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow sm:text-sm border-gray-400 rounded-md">
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" selected="{{  $book->category_id  }}">{{ $category->name }}</option>
                        @endforeach
                    </select>
                    @error('category_id')<span class="mt-2 text-sm text-red-500">{{ $message }}</span>@enderror
                </div>
              ...
              <div>
                ...
                <div class="form-input mt-1">
                  <textarea id="description" name="description"  rows="3" class="shadow focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border border-gray-400 rounded-md">
                    {{ $book->description }}
                  </textarea>
                </div>
              ...
              <div class="col-span-6 sm:col-span-3">
                ...
                <input type="number" name="pub_year" value="{{ $book->pub_year }}" min="1600" max="2100" id="pub_year" required autocomplete="pub_year" class="form-input mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow sm:text-sm border-gray-400 rounded-md">
